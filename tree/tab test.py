...
# ======================================================================================================================
def read_cords_tab6():
    cord_file = 'coords.txt'
    f = open(cord_file, "r")  # open this file
    ...
    for line in f.readlines():  # read lines
        ...
        if field == 0:
            cords.append({})  # add a list item consisting of an empty dictionary
            cords[-1]['xyz'] = line
            field = 0
    
    f.close()        
            
    # this code displays the contents that was read from the file
    for cord in cords:
        guest_details = (cord_list_tab6.get() + '\n' + cord['Title'])
...
ttk.Label(tab1, text="temp").grid(column=1, row=1, sticky=E)
